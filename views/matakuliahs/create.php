    ...
    <div class="max-w-lg w-full bg-gradient-to-r from-blue-50 to-blue-100 p-1 rounded-2xl shadow-lg">
        <div class="bg-white p-8 rounded-xl shadow-md">
            <h2 class="text-3xl font-extrabold mb-6 text-center text-blue-600 flex items-center justify-center gap-2">
                <i data-lucide="book-open" class="w-7 h-7"></i> Tambah Mata Kuliah
            </h2>
            ...
            <form action="/project/matakuliah/store" method="POST" class="space-y-5">
                <input type="hidden" name="csrf_token" value="<?= $auth->generateCsrfToken(); ?>">
                <div>
                    <label class="block mb-2 font-medium text-gray-700 flex items-center gap-2">
                        <i data-lucide="hash" class="w-4 h-4 text-blue-500"></i> Kode Mata Kuliah
                    </label>
                    <input type="text" name="kode_matkul"
                        value="<?= htmlspecialchars($old['kode_matkul'] ?? ''); ?>"
                        required
                ...
                <div>
                    ...
                    <input type="text" name="nama_matkul"
                        value="<?= htmlspecialchars($old['nama_matkul'] ?? ''); ?>"
                        required
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                </div>
                <div>
                    ...
                    <input type="text" name="sks"
                        value="<?= htmlspecialchars($old['sks'] ?? ''); ?>"
                        required
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                </div>
                <div>
                    ...
                    <input type="text" name="semester"
                        value="<?= htmlspecialchars($old['semester'] ?? ''); ?>"
                        required
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                </div>
